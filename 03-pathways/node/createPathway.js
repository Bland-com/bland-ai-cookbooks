...
// Note: Bland does not use a "Bearer" prefix, just the raw key.
const HEADERS = {
 ...
 */
async function createEmptyPathway() {
  ...
  const pathwayId = response.data.pathway_id;
  console.log(`Pathway created successfully. ID: ${pathwayId}`);
 ...
 *       - extractVars:   Array of [varName, varType, varDescription] tuples.
 *       - modelOptions:  Fine-tune the AI model: modelName, interruptionThreshold, temperature
 */
function buildNodes() {
  const nodes = [
    // -----------------------------------------------------------------
      ...
      data: {
        ...
        extractVars: [
          ...
          [
            "date",
            "string",
            "The date of the reservation (e.g., 'March 15th', 'next Friday')",
          ],
          [
            "time",
            "string",
            "The time of the reservation (e.g., '7pm', '6:30 in the evening')",
          ],
          [
            "party_size",
            "string",
            "The number of guests (e.g., '4', 'two people', 'a party of six')",
          ],
        ],
      },
    },
    ...
    // -----------------------------------------------------------------
    {
      ...
      data: {
        ...
        globalLabel:
          "The caller asks a general question about the restaurant (hours, menu, location, parking, etc.)",
        prompt:
...
function buildEdges() {
  const edges = [
    ...
    // The caller is done after the greeting (maybe they just had a quick
    // question handled by the global node, or they called the wrong place).
    {
      id: "edge_1_5",
      source: "1", // Greeting
      target: "5", // End Call
    ...
    // This edge only activates after the node's condition is met.
    {
      ...
      label:
        "The caller has provided the date, time, and party size for the reservation",
    },
    ...
    // The caller wants to change something (wrong date, different time, etc.)
    {
      ...
      target: "2", // Reservation Details
      label:
        "The caller wants to change the reservation details (date, time, or party size)",
    },
  ];
 ...
 */
async function updatePathwayWithStructure(
  pathwayId,
  nodes,
  edges,
  globalConfig
) {
  const url = `${BASE_URL}/pathway/${pathwayId}`;
  ...
  console.log(
    `\nUpdating pathway ${pathwayId} with ${nodes.length} nodes and ${edges.length} edges...`
  );
  const response = await axios.post(url, payload, { headers: HEADERS });
...
async function main() {
  ...
  try {
    ...
    const pathwayId = await createEmptyPathway();
    ...
    await updatePathwayWithStructure(pathwayId, nodes, edges, globalConfig);
    ...
    console.log(
      `Global nodes:  ${nodes.filter((n) => n.data.isGlobal).length}`
    );
    console.log(`Variables:     date, time, party_size`);
    console.log(`\nUse this pathway_id to send calls:`);
    console.log(`  node sendCallWithPathway.js`);
